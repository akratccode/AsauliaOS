...
export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations('contractor.clients');
...
export default async function ContractorClientsPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-5xl space-y-6 p-6">
      <header>
        <p className="text-fg-3 text-xs uppercase tracking-[0.12em]">{t('assignmentsLabel')}</p>
        <h1 className="text-fg-1 font-serif text-3xl italic">{t('clientsTitle')}</h1>
      </header>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {rows.map((r) => (
            ...
            >
              ...
              <div className="text-fg-3 mt-1 text-xs">{t('role')} {r.role}</div>
              <div className="text-fg-2 mt-3 text-sm">
                {/* eslint-disable-next-line i18next/no-literal-string -- formatted cents value appended */}
                {t('projectedThisPeriod')} · {formatCents(earningsByBrand.get(r.brandId) ?? 0)}
              </div>
            </Link>
          ))}
        </div>
