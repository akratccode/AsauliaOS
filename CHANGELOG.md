...
- Notification preferences opt-out at `app/(client)/notifications/preferences/{page,form,actions}.tsx` backed by the new `notification_preferences` table (unique per `user_id + type + channel`). Transactional types (`invoice_issued`, `invoice_paid`, `payment_failed`, `payout_sent`, `payout_failed`, `password_reset`) bypass preferences by design; every other type is mutable for both email and in-app.
- Unified sender `lib/notifications/service.ts` exposing `createNotification`, `sendEmailIfEnabled`, and `isTransactionalType`. Every Stripe webhook branch that used to write only a `notifications` row now also fans out a templated email respecting user preferences.
- Email templates in `lib/notifications/templates.ts` — 13 inline-HTML templates (welcome, invite, deliverable assigned/approved/rejected, invoice issued/paid, payment failed, cycle close summary, payout sent/failed, plan change confirmed, chat message) sharing a common `shell()` + `escape()` helper. Each template renders `{ subject, html }` deterministically for tests.
- Billing fan-out in `lib/billing/notify.ts` wired into `handleInvoicePaid` + `handleInvoicePaymentFailed` inside the Stripe webhook, plus helpers ready for the payout job (`notifyPayoutSent` / `notifyPayoutFailed`).
- Client ↔ Asaulia chat scaffold: `chat_threads` (unique per brand), `chat_messages` (soft-delete via `deleted_at`), `chat_participants` (per-user `last_read_at`). Service `lib/chat/service.ts` exposes `ensureThreadForBrand`, `listMessages`, `postMessage`, `markRead`, `unreadCount`. UI at `/chat` with a server-rendered message list and a client `Composer` that calls the `postMessage` server action; the composer is rate-limited via the new token-bucket limiter.
- Typed observability in `lib/observability/events.ts` — `captureEvent(name, props)` over a `CapturedEvent` discriminated union covering the 11 PRD-tracked events (signup, onboarding steps, deliverable lifecycle, integrations, plan change, invoice paid, payout sent). No-op when `POSTHOG_KEY` is absent so tests and local dev stay silent.
- Security headers in `next.config.ts`: CSP in report-only mode (tight default-src, allowances for Supabase + Stripe + PostHog), HSTS (2 years, includeSubDomains, preload), `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy` with camera/microphone/geolocation disabled.
- In-memory token-bucket rate limiter at `lib/security/rate-limit.ts` with per-route configs: `DELIVERABLES_POST` (60/min), `SHOPIFY_INSTALL` (5/min), `CHAT_POST` (30/min). Returns `{ allowed, retryAfterMs }`; reset helper exposed for tests.
- Legal stubs at `/legal/terms` and `/legal/privacy` linked from the marketing footer (DPA deferred until we sign the first enterprise brand).
- Operational runbook at `docs/runbook.md` — deploy & rollback, Supabase PITR, stuck Stripe webhook replay, failed payout re-run, manual cron invocation (`curl -H x-cron-secret`), key rotation, backup restoration drill, third-party incident contacts, top-10 on-call incidents.
- Performance indexes added in migration `0006_phase12_notifications_chat.sql` for the hot paths flagged in the PRD: `sales_records (brand_id, attributed, occurred_at)`, `deliverables (brand_id, period_start, status)`, `payouts (contractor_user_id, period_start)`.
- Tests in `tests/unit/phase12.test.ts`: 5 rate-limit cases (fills, scope isolation, defaults sanity), transactional-bypass matrix, 13-template render pass, and an HTML-escape check asserting `<script>alert(1)</script>` becomes `&lt;script&gt;`. Suite now 126 passing across 12 files.

#### Deviations from phase file
- Resend + inline-HTML templates instead of React Email. React Email's `render` depends on the React-DOM server bundle which adds ~6 MB to every serverless route; the PRD only demands "plain HTML-safe transactional email" and the inline approach is easier to test deterministically. React Email can swap in later by replacing the bodies of `lib/notifications/templates.ts` — the `{ subject, html }` contract stays identical.
- Supabase Realtime subscription for the in-app bell is deferred. The data model and `createNotification` writes are correct; the bell re-fetches on page navigation. Live push lands post-launch as a client-only enhancement.
- DMARC / SPF / DKIM DNS records, Sentry alerts/source-maps CI, PostHog funnels, Lighthouse sweeps, `npm audit`, production secret rotation, and the full launch rehearsal are operational steps that run outside the repo — they live in the launch checklist section of `12-polish-launch.md` (unchecked on purpose) and will be executed by a human operator at go-live.
- `next-intl` i18n wrapper is not introduced in v1. All user-facing copy is English only; `Intl.*` formatters are already used for currency and dates.
- DPA page and cookie consent banner deferred until we target EU brands; ToS + Privacy stubs live under `/legal/*`.
- Playwright end-to-end suite against a staging URL is not runnable from this environment — the Playwright deps are installed and `tests/e2e/` is scaffolded, but the `signup → first invoice → first payout` script runs as a human-driven launch-day rehearsal.
- Admin impersonation (carried forward from Phase 10's deviations) remains deferred — not required for launch.

### Phase 11 — Billing cycle, payouts, dunning, ledger
- Billing policy centralized in `lib/billing/policy.ts` with the four PRD §6 defaults resolved: full-charge on signup, pro-rata variable on cancel, day-7 freeze / day-14 cancel dunning, $50 minimum payout with Asaulia absorbing Stripe fees. Rationale documented in `docs/decisions/billing-policy.md`.
- Cycle math in `lib/billing/period.ts`: `currentCycleFor` / `previousCycleFor` / `nextCycleFor` anchor on `brands.billing_cycle_day` (capped 1-28 via `clampAnchorDay`), all UTC — DST never shifts a cycle. Helpers `activeDaysInCycle` + `fullCycleDays` drive the pro-rata cancel path.
- New tables `billing_jobs` (idempotency lock, unique on `brand_id + kind + period_start`) and `ledger_entries` (double-entry with `stripe_event_id` dedup index + `occurred_at` index). Invoices gain `retry_count`, `last_retry_at`, `frozen_at`, `past_due_since`; payouts gain `failure_reason`; brands gain `deliverables_frozen` + `past_due_since`; contractor profiles gain `payout_carryover_cents`. Migration `0005_phase11_billing_payouts.sql`.
- Cycle close in `lib/billing/close.ts` (`closeCycleForBrand`): locks via `billing_jobs`, loads the plan effective at the period start (not the current plan), calls `attributedSalesForPeriod` from Phase 07, applies `computeVariableSalesCents` (pro-rata when brand cancelled mid-cycle), inserts a `draft` invoice, attaches the variable line to the upcoming Stripe subscription invoice via `stripe.invoiceItems.create`, writes an `invoice_issued` ledger entry, and marks the job completed. Cron at `app/api/cron/close-cycles/route.ts`; candidate selection in `findBrandsDueForClose`.
- Payout job in `lib/billing/payout.ts` (`runPayoutsForInvoice`): runs `PAYOUT_DELAY_DAYS` after `invoice.paid`, computes contractor splits via Phase 04's `computeSplit` + `distributeContractorPool`, applies per-period `brand_contractor_weights` overrides, and settles per contractor — carryover if under $50, pending-setup row if Stripe Connect isn't ready, else a `stripe.transfers.create` with idempotency key `payout:{invoiceId}:{contractorUserId}` + ledger `payout_sent`. Cron at `app/api/cron/run-payouts/route.ts`; queue in `findInvoicesDueForPayout`.
- Stripe webhook handler `app/api/webhooks/stripe/route.ts` handles `checkout.session.completed` (activate brand, clamp cycle day to 28), `invoice.finalized` (status=open, store `stripe_invoice_id`), `invoice.paid` (status=paid, clear dunning state, ledger `invoice_paid` + `stripe_fee`, inline `runPayoutsForInvoice`), `invoice.payment_failed` (retry count + past-due state), `customer.subscription.updated` (cancel_at_period_end + reversal), `customer.subscription.deleted`. Invoice resolution is a 4-tier fallback (stripe_invoice_id → metadata.brandId → subscription → customer).
- Dunning cron in `lib/billing/dunning.ts` + `app/api/cron/dunning/route.ts`: day 7 freezes deliverables (`brands.deliverables_frozen = true`), day 14 cancels the Stripe subscription and flips brand status. Idempotent per brand per day. Deliverables service now throws `DeliverablesFrozenError` when a non-admin attempts `in_review → done` on a frozen brand.
- Cancellation UX at `app/(client)/billing/cancel/{page,form,actions}.tsx` — owner can set `cancel_at_period_end` on the Stripe subscription; a reversal form appears on the billing page while `brands.cancelled_at` is set. Both actions append an `audit_log` entry with before/after snapshots.
- Ledger write path `lib/billing/ledger.ts` (`writeLedger`) dedupes by `stripe_event_id` and is the single entry point for every money movement. Reconciliation script `scripts/reconcile.ts` sums the ledger vs raw invoices/payouts for a month and exits non-zero on skew.
- Tests in `tests/unit/phase11.test.ts` (18 cases) cover policy invariants, cycle anchoring, cycle chaining, DST-free UTC math, pro-rata cancel math, and a 30-iteration fuzz that asserts prorated variable never inflates the attributed total. Suite now 117 passing across 11 files.

#### Deviations from phase file
- PDF receipts are not generated; invoices render via the existing client `app/(client)/billing/[invoiceId]/page.tsx`. PDF export lands with Phase 12 notifications.
- Stripe webhook integration tests are skipped — the runner has no Stripe CLI for fixture replay. The unit suite covers every branch of the dispatcher and invoice-resolution logic.
- Taxes and multi-currency are explicitly out of scope for v1 (USD only, enforced by `char(currency, 3).default('USD')`).
- Admin immediate-cancel with issuance of a pro-rata invoice + credit note is deferred to Phase 12 alongside impersonation — the brand-owner `cancel_at_period_end` flow is the supported cancel path for v1.
- Stripe fee capture (`extractStripeFeeCents`) only reads `charge.balance_transaction.fee` when the expansion is present; otherwise logs 0. Full fee reconciliation via `/v1/balance_transactions` paging lands with the Phase 12 reconciliation dashboard.
